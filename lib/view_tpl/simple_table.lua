...
return function(data1, macro1, obj, params)
    ...
    macro = json_decode(base64_decode(macro1))

    --out('---------data1:', page, per_page, total, data)

    --out('---------macro:', macro.edit)
    --out('---------obj:', obj)
    --out('---------params:', params)
    --out('---------data:', data)
...
%s
    ...
    for j, t in pairs(data[obj]) do
        ...
        for i, _ in pairs(params) do
            --out('it+', j, i)
            print('***************', json_encode(macro[i] or {}), i)
            if is_valid(macro[i]) and macro[i].param and macro[i].code then
                print('--------macro valid-----------')
                ...
                print('FUNC:====================2', err, r, func)
                print('FUNC:====================3',t.id,t.package_name,t.name)
