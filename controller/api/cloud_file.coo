...
function class_member:upload()
    ...
    if _is_valid(p['file[]']) then
        ...
        for i, v in pairs(p['file[]']) do
            ...
            -- 已经存在目录中的文件，可能是引用
            if _is_valid(find_exit_already_have) and find_exit_already_have.md5 ~= cftmp.md5 then
                -- 删除文件之前先判断是否还有其他引用
            ...
            if _copy(v['temp'], to_be_save) then
                if not cftmp:save() then
                    print('save cloud_file db fail:==', _json_encode(cftmp.snap))
                end
            else
